...
function printColoredLines(content, wordCount) {
    ...
    for (const line of lines) {
        const cleanedLine = line.replace(/[!.,;:?"'()-]/g, ' '); // Remove specific punctuation marks
        const coloredLine = cleanedLine.split(/\s+/).map(word => {
            const lowerWord = word.toLowerCase();
            return colorWord(word, wordCount[lowerWord] || 0);
        }).join(' ');
